...
class MySQL_horario {
    ...
    const TABLA = "fechas"; // Nombre de la tabla
    ...
    private function _modificarDatos(Sql_horario $sql, &$mensaje) {
    ...
    public function modificar(Sql_horario $sql, &$mensaje) {
        $datos = $this->_modificarDatos($sql, $mensaje);
        return $datos;
    }

    private function _traerDatos(Sql_horario $sql, &$mensaje) {
        $ejecucion = $sql->generarEjecucion();
        $datos = array();
        try {
            $this->_conectar();
// aunque $sql es un objeto, el método __toString de la clase Sql devuelve una cadena
    ...
    public function ejecutar(Sql_horario $sql, &$mensaje) {
