<!--
    ...
    Licencia = GPL v3
    Versión = 1.0
    Descripción = MySQL_cuenta, se encarga de conectar con la BBDD
...
class MySQL_cuenta {
    ...
    private function _modificarDatos(Sql_cuenta $sql, &$mensaje) {
    ...
    public function modificar(Sql_cuenta $sql, &$mensaje) {
        $datos = $this->_modificarDatos($sql, $mensaje);
        return $datos;
    }

    private function _traerDatos(Sql_cuenta $sql, &$mensaje) {
        $ejecucion = $sql->generarEjecucion();
        $datos = array();
        try {
            $this->_conectar();
// aunque $sql es un objeto, el método __toString de la clase Sql devuelve una cadena
    ...
    public function ejecutar(Sql_cuenta $sql, &$mensaje) {
